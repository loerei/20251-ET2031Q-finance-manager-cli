// Finance Manager v1.3 - Personal Finance Management System
// 
...
// 
// Build: g++ -std=c++17 finance_v1_3.cpp -o finance_v1_3
// Usage: ./finance_v1_3 (interactive mode)
...
struct Account {
    ...
    void allocateAmount(const chrono_tp &date, double amount, const string &note) {
        ...
        if (totalPct <= 0.000001) {
            ...
            txs.push_back({date, amount, "Other", note + " (auto alloc fallback)"});
            balance += amount;
            return;
        }
        for (auto &p : allocationPct) {
            ...
            txs.push_back({date, share, p.first, note + " (auto alloc)"});
            balance += share;
        }
    }
    ...
    void processSchedulesUpTo(const chrono_tp &upTo) {
        for (auto &s : schedules) {
            ...
            while (s.nextDate <= upTo && guard < 10000) {
                if (s.autoAllocate) allocateAmount(s.nextDate, s.amount, "Scheduled income: " + s.note);
    ...
    void applyMonthlySavingInterest() {
        ...
        chrono_tp dt = today();
        Transaction tt; tt.date = dt; tt.amount = interest; tt.category = "Saving"; tt.note = "Monthly interest";
        txs.push_back(tt);
balance += interest;
    }
    ...
    while (true) {
        ...
        if (choiceStr == "H" || choiceStr == "h") {
            ...
            if (choice == 1) {
                ...
                if (amt > 0.0 && autoAlloc) acc.allocateAmount(today(), amt, note + " (manual income)");
