// Finance Manager v2.0 - Personal Finance Management System
// 
...
// 
// Build: g++ -std=c++17 finance_v2_0.cpp -o finance_v2_0
// Usage: ./finance_v2_0 (interactive mode)
...
struct Account {
    ...
    void ensureCategoryExists(const string &display) {
    ...
    void setAllocation(const map<string,double> &newAlloc) {
        // Replace full allocation mapping, but only for keys that already exist in displayNames.
        allocationPct.clear();
        for (auto &p : newAlloc) {
            string nk = normalizeKey(p.first);
            allocationPct[nk] = p.second;
