// Finance Manager v2.2 - Personal Finance Management System
// 
...
// 
// Build: g++ -std=c++17 finance_v2_2.cpp -o finance_v2_2
// Usage: ./finance_v2_2 (interactive mode)
...
static inline string tr(const Settings &s, const string &id) {
    ...
    if (id == "press_enter") return vi ? "Nhấn Enter để quay lại menu chính.\n" : "Press Enter to go back to main menu.\n";
...
int main() {
    ...
    if (!loaded) {
        if (acc.settings.language == "VI") cout << "Không thể mở tệp để tải. Không tìm thấy lưu hiện có trong thư mục làm việc.\n";
    ...
    while (true) {
        ...
        if (choiceStr == "H" || choiceStr == "h") {
            ...
            if (choice == 1) {
                ...
                if (acc.settings.language == "VI") cout << "Nhập loại hoặc số của nó, hoặc để trống để tự động phân bổ (cho số dương).\n";
                else cout << "Enter category or its number, or leave blank for Auto-allocate (for positive amounts).\n";
                if (acc.settings.language == "VI") cout << "Các loại hiện có:\n"; else cout << "Existing categories:\n";
                for (size_t i = 0; i < cats.size(); ++i) {
                    cout << "  " << (i+1) << ". " << cats[i].first << "\n";
                }
                if (acc.settings.language == "VI") cout << "Loại (tên hoặc số) [trống = Auto-allocate khi dương]: ";
                else cout << "Category (name or number) [blank = Auto-allocate when positive]: ";
                    ...
                    if (isNumber) {
                        try {
                            ...
                            if (idx >= 1 && idx <= (int)cats.size()) {
                                ...
                                if (acc.settings.language == "VI") cout << "Số ngoài phạm vi.\n"; else cout << "Number out of range.\n";
                            }
                        } catch (...) {}
                    }
                    if (!handled) {
                        ...
                        if (acc.displayNames.find(nk) != acc.displayNames.end()) {
                            ...
                            while (true) {
                                ...
                                if (!resp.empty() && (resp[0]=='c' || resp[0]=='C')) {
                                    ...
                                    if (acc.settings.language == "VI") cout << "Nhập lại loại (tên hoặc số) [trống = Auto-allocate khi dương]: ";
                                    else cout << "Retype category (name or number) [blank = Auto-allocate when positive]: ";
                                    if (!getline(cin, catInput)) catInput.clear();
                                    trim_inplace(catInput);
                                    if (catInput.empty()) {
                ...
                if (!sub.empty() && (sub[0]=='a' || sub[0]=='A')) {
                    ...
                    if (acc.settings.language == "VI") cout << "Chọn loại(hoặc các loại) áp dụng lãi (số cách nhau bởi dấu phẩy, hoặc tên cách nhau dấu phẩy): ";
                    else cout << "Please choose which category(/ies) to apply interest rate (number(s) separated by comma, or names separated by comma): ";
                    string catSel; if (!getline(cin, catSel)) catSel.clear();
                    trim_inplace(catSel);
                    if (catSel.empty()) { if (acc.settings.language == "VI") cout << "Không có loại được chọn.\n"; else cout << "No categories selected.\n"; if (!askReturnToMenuOrSave(acc)) break; else continue; }
                    // split on commas
                        ...
                        if (isNumber) {
                            try {
                                ...
                                if (idx >= 1 && idx <= (int)cats.size()) {
                                    ...
                                    if (acc.settings.language == "VI") cout << "Số ngoài phạm vi cho lựa chọn: " << s << "\n";
                                    else cout << "Number out of range for selection: " << s << "\n";
                                    continue;
                                }
                            } catch (...) { continue; }
                        } else {
                            ...
                            if (acc.displayNames.find(nk) != acc.displayNames.end()) {
                                ...
                                if (acc.settings.language == "VI") cout << "Không tìm thấy loại: " << s << " (-> " << sanitized << "). Bỏ qua.\n";
                                else cout << "Category not found: " << s << " (sanitized -> " << sanitized << "). Skipping.\n";
                                continue;
                            }
                        }
                    }
                    if (targets.empty()) { if (acc.settings.language == "VI") cout << "Không có loại hợp lệ được chọn.\n"; else cout << "No valid categories selected.\n"; if (!askReturnToMenuOrSave(acc)) break; else continue; }

                    // Ask monthly or annual
                    if (acc.settings.language == "VI") cout << "(M) hàng tháng hay (A) hàng năm? [m/a]: ";
                    else cout << "(M)onthly or (A)nnually? [m/a]: ";
                    string ma; if (!getline(cin, ma)) ma = "m";
                    trim_inplace(ma);
                    bool monthly = true;
                    if (!ma.empty() && (ma[0]=='a' || ma[0]=='A')) monthly = false;
                    // Ask for rate value
                    if (acc.settings.language == "VI") cout << "Nhập lãi (ví dụ '0.5%', '0,5', '5.5', '5,5%'): ";
                    else cout << "Enter rate (numbers accepted like '0.5%', '0,5', '5.5', '5,5%'): ";
                    string rateIn; if (!getline(cin, rateIn)) rateIn.clear();
                    trim_inplace(rateIn);
                    double ratePct = 0.0;
                    if (!tryParseRate(rateIn, ratePct)) {
                        if (acc.settings.language == "VI") cout << "Dữ liệu lãi không hợp lệ.\n"; else cout << "Invalid rate input.\n";
                        if (!askReturnToMenuOrSave(acc)) break; else continue;
                    }
                    // Ask for start date
                    if (acc.settings.language == "VI") cout << "Lãi được áp dụng từ ngày (YYYY-MM-DD) [trống = hôm nay]: ";
                    else cout << "The interest rate is applied since (YYYY-MM-DD) [leave blank = today]: ";
                    ...
                    if (!startIn.empty()) {
                        if (!tryParseDate(startIn, startDate)) {
                            if (acc.settings.language == "VI") cout << "Ngày bắt đầu không hợp lệ.\n"; else cout << "Invalid start date.\n";
                            if (!askReturnToMenuOrSave(acc)) break; else continue;
                        }
                    }
                    ...
                    for (string nk : targets) {
                        ...
                        if (acc.settings.language == "VI") cout << "Đặt lãi " << (monthly ? "hàng tháng" : "hàng năm") << " " << ratePct << "% cho loại '" << acc.displayNames[nk] << "' bắt đầu " << toDateString(startDate) << ".\n";
                        else cout << "Set " << (monthly ? "monthly" : "annual") << " rate " << ratePct << "% for category '" << acc.displayNames[nk] << "' starting " << toDateString(startDate) << ".\n";
                    }
                } else if (!sub.empty() && (sub[0]=='r' || sub[0]=='R')) {
                    // remove interest entries
                    if (acc.interestMap.empty()) { if (acc.settings.language == "VI") cout << "Không có mục lãi để xóa.\n"; else cout << "No interest entries to remove.\n"; if (!askReturnToMenuOrSave(acc)) break; else continue; }
                    if (acc.settings.language == "VI") cout << "Các mục lãi:\n"; else cout << "Interest entries:\n";
                    ...
                    if (acc.settings.language == "VI") cout << "Nhập số (các số cách nhau dấu phẩy) để xóa: ";
                    else cout << "Enter number(s) to remove (comma separated): ";
                    string rem; if (!getline(cin, rem)) rem.clear();
                    trim_inplace(rem);
                    if (rem.empty()) { if (acc.settings.language == "VI") cout << "Không chọn gì.\n"; else cout << "No selection made.\n"; if (!askReturnToMenuOrSave(acc)) break; else continue; }
                    ...
                    for (auto &t : tokens) {
                        try {
                            ...
                            for (auto &p : idxToNk) if (p.first == idx) {
                                ...
                                if (acc.settings.language == "VI") cout << "Đã xóa lãi cho " << (acc.displayNames.count(p.second) ? acc.displayNames[p.second] : p.second) << "\n";
                                else cout << "Removed interest for " << (acc.displayNames.count(p.second) ? acc.displayNames[p.second] : p.second) << "\n";
                            }
                        } catch (...) {}
                    }
                } else {
                    // process / apply interest now
                    if (acc.settings.language == "VI") cout << "Đang xử lý & áp dụng lãi đến hôm nay (" << toDateString(today()) << ")...\n";
                    else cout << "Processing & applying interest up to today (" << toDateString(today()) << ")...\n";
                    acc.applyInterestUpTo(today());
                    if (acc.settings.language == "VI") cout << "Đã áp dụng lãi.\n"; else cout << "Interest applied.\n";
                }
                ...
                cout << (acc.settings.language == "VI" ? "Tạm biệt!\n" : "Goodbye!\n");
                didExit = true;
            } else if (choice == 10) {
                // Enter settings. settingsMenu uses getline internally so no extra newline issues.
                settingsMenu(acc);
            } else {
                cout << (acc.settings.language == "VI" ? "Lựa chọn không hợp lệ.\n" : "Invalid choice.\n");
            }
